...
async function fetchDataInParallel(req) {
	const query_list = req.csca.query_list;
	const promise_list = [];
...
async function parseDataInParallel(req) {
	...
	const promise_list = funcs.map((func) => (new Promise((resolve) => {
		func(req);
		resolve();
	})));
...
function parseLanguageRules(req) {
	const dummy_raw_data_freshman_one = {
		cos_cname:	'大一英文（一）',
		cos_ename:	'Freshman English (I)',
		cos_type:	'外語',
		cos_codes:	[]
	};
	const dummy_raw_data_freshman_two = {
		cos_cname:	'大一英文（二）',
		cos_ename:	'Freshman English (II)',
		cos_type:	'外語',
		cos_codes:	[]
	};
	const dummy_raw_data_advanced = {
		cos_cname:	'進階英文',
		cos_ename:	'',
		cos_type:	'外語',
		cos_codes:	[]
