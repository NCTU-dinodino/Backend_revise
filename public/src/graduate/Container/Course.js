class Course {
	...
	split(time_id) {
		const split_course = Object.assign(new Course(), this);
		split_course.data = {};
	...
	copy(){
		let result = Object.assign(Course(), this);
		result.data = {}
	...
	format(student_id) {
		const representing_data = this.getRepresentingData();
		const result = {
			cn:	representing_data.cname,
			en:	representing_data.ename,
			scores:	this.is_dummy ? [] : Object.values(this.data).map((data) => (data.score)).map((score) => ({
				score: score.score,
				grade: score.grade,
