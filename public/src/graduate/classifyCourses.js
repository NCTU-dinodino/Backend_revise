...
function handleCompulsory(req) {
	...
	req.csca.rules.compulsory.course_rules.forEach((rule) => {
		...
		if (rule.cname.startsWith('物化生')) {
			...
			let first_passed_course, passed_idx;
			...
			if (rule.ext.physic.length > 1) {
				...
				rule.ext.physic.forEach((course) => {
					course.moved = true;
					if (course.type == '通識') req.csca.classes.general_old.courses.push(course);
					else req.csca.classes.elective.courses.push(course);
				});
			...
			if (passed_idx == -1)passed_idx = 0;
			const first_passed_course = rule.courses[passed_idx];
...
function formatCompulsory(req) {
	...
	req.csca.rules.compulsory.course_rules.forEach((rule) => {
		if (rule.courses.length == 0) req.csca.classes.compulsory.courses.push(rule.createEmptyCourse(req.csca.student_id));
		else req.csca.classes.compulsory.courses.push(...rule.courses);
	});
}
...
function handleLanguage(req) {
	switch (req.csca.data.user_info.en_certificate) {
		...
		case '2': case '3': case '4': {
			const empty_advanced = req.csca.rules.language.advanced.createEmptyCourse();
			while (req.csca.classes.language.courses.length < 4) req.csca.classes.language.courses.push(empty_advanced);
			break;
		}
		default: {
			...
			for (let i = 0; i < 2; i++) {
				const representing_data = advanced[i].getRepresentingData();
...
function splitSamelyCodedCourse(req) {
	...
	course_list.forEach((course_detail) => {
		const course_class = req.csca.classes[course_detail['class']];
